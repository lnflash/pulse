//! Receive payment command handler
...
#[async_trait]
impl CommandHandler for ReceiveHandler {
    async fn handle(
        &self,
        ...
        // Parse amount if provided (optional for variable amount invoices)
        ...
        match self.flash_api.create_invoice(&auth_token, amount_sats, memo).await {
            Ok(invoice) => {
                ...
                Ok(CommandResponse::text(format!(
                    ...
                    Payment Hash: {}\n\n\
                    Payment Request:\n```{}```\n\n\
                    ⚡ Scan the QR code or copy the payment request to pay.",
